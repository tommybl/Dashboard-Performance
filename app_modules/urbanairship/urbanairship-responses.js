...
function getDatasSum (datas, type) {
    var sumIos = 0, sumAnd = 0;
    for (var i = 0; i < datas.length; i++) {
        var ios = (datas[i])['ios'][type];
        ios = (ios == -1) ? 0 : ios;
        var android = (datas[i])['android'][type];
...
exports.query = function (codeName, route, type, start, end, granularity, past, session, client, res, limiter) {
    ...
    function pastCallback (response) {
        ...
        if ((response[0])['next_page'] != undefined) {
            var promises = [];
            datasNow = datasNow.concat((response[0])['responses']);
            datasPast = datasPast.concat((response[1])['responses']);
        ...
        // if the datas retrieved are on the last page we format all the datas cumulated in the arrays and put them in the response
            ...
            datasNow = getDatasSum(datasNow, type);
            datasPast = getDatasSum(datasPast, type);
            var result = {
                code: "OK",
                codeName: codeName,
                ios: {
    ...
    function nowCallback (error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            if (datas['next_page'] != undefined) {
                datasNow = datasNow.concat(datas['responses']);
            ...
            // if the datas retrieved are on the last page we format all the datas cumulated in the arrays and put them in the response
                ...
                datasNow = datasNow.concat(datas['responses']);
                if (!hasStep) {
                    datas = getDatasSum(datasNow, type);
                    var result = {
                        code: "OK",
                        codeName: codeName,
                        ios: {
                ...
                // translatting datas from urban airship datas format to our own datas format
                    ...
                    for (var i = 0; i < datas.length; i++) {
                        ...
                        ios = (datas[i])['ios'][type];
                        android = (datas[i])['android'][type];
